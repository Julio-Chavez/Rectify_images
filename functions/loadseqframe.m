function [im,headerInfo] = loadseqframe(fileName,n)
...
%    OUTPUTS:
%    HEADERINFO: A structure containing Norpix version, header size (bytes),
...
%    IM:    an ImageHeight x ImageWidth image
%
% SYNTAX:
...
% 2020           Michelle DiBenedetto modified to read monochrome images 
%                and slightly modified syntax to only load in 1 frame at a time
 ...
 %   numFramesToProcess = 2;
  ...
  %  for i = 1:numFramesToProcess
        ...
        switch headerInfo.ImageFormat
            ...
            UYVYcolVec = fread(fid, numPixels, bitstr, endianType); %mimi changed numpixels/2

            if length(UYVYcolVec)==0
                disp('frame number doesn''t exist')
                im=[];
               return
            end
